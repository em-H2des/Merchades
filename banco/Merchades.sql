...
BEGIN
    SET NOCOUNT ON;
    -- Insere na tabela CLIENTE pelo cpf
    INSERT INTO CLIENTE (CPF_CNPJ_CLIENTE)
    
    -- Pega os cpfs que nao existem
    SELECT DISTINCT 
        i.CPF_CNPJ_VENDA 
    FROM 
        INSERTED i --pega os cpfs pela nota que acabou de ser insertada
    LEFT JOIN 
        CLIENTE c ON i.CPF_CNPJ_VENDA = c.CPF_CNPJ_CLIENTE 
    WHERE c.CPF_CNPJ_CLIENTE IS NULL; -- Só insere se não tiver o cliente
END;
GO 
